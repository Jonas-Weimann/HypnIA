...
const getAllCartas = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error obteniendo cartas", error: error.message });
  }
};
...
const getCartaById = async (req, res) => {
  ...
  try {
    ...
    const carta = await dbClient.query(
      "SELECT * FROM cartas WHERE id_carta = $1",
      [cid]
    );
    ...
    res
      ...
      .json({ message: error.message || "Error obteniendo carta" });
  }
};

const createCarta = async (req, res) => {
  const { nombre, descripcion, imagen, elemento, polaridad, emociones } =
    req.body;
  ...
  try {
    ...
    const insertarEmociones = emociones.map((emocion) =>
      dbClient.query(
        "INSERT INTO cartas_emociones (id_carta, id_emocion) VALUES ($1, $2)",
        [idCarta, emocion]
      )
    );
    ...
    res
      .status(error.status || 500)
      .json({ message: error.message || "Error creando carta" });
  }
};
...
const updateCarta = async (req, res) => {
  ...
  try {
    ...
    await dbClient.query("DELETE FROM cartas_emociones WHERE id_carta = $1", [
      idCarta,
    ]);
    const insertarEmociones = emociones.map((emocion) =>
      dbClient.query(
    ...
    res
      .status(error.status || 500)
      .json({ message: error.message || "Error actualizando carta" });
  }
};
...
const deleteCarta = async (req, res) => {
  ...
  try {
    ...
    const cartaEliminada = await dbClient.query(
      "DELETE FROM cartas WHERE id_carta = $1 RETURNING *",
      [cid]
    );
    ...
    res
      .status(500)
      .json({ message: "Error eliminando carta", error: error.message });
  }
};

export { getAllCartas, getCartaById, createCarta, updateCarta, deleteCarta };
